...
\textbf{Q1.} - Using a subquery and a case when statement, categorise of the 2023 London Marathon in the following way: >3 hours; 02:30:00 to 3hours; below 02:30:00 ? Proivde the runner's full name, date_of_bith, sex, and country in the query result?

\textbf{Solution} - 

\textbf{SELECT r.first\_name || ' ' || r.last\_name AS full\_name, r.country, r.sex, re.finish_time, CASE WHEN re.finish_time  FROM Runners r
INNER JOIN Results re ON re.runner_id = r.runner_id WHERE re.event_id IN (SELECT e.event_id FROM Events e WHERE event = 'London Marathon' AND event_date = '2023-04-23')

r.event\_id IN ( SELECT sa.runner\_id FROM Sponsored\_Athletes sa WHERE sa.sponsor\_name = 'Nike' OR sa.sponsor\_name = 'Tracksmith');}