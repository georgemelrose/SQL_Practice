...
\textbf{SELECT re.finish_time, r.first\_name || ' ' || r.last\_name AS full\_name, r.date\_of\_birth, r.sex, r.country, tpd.plan\_description FROM Runners r INNER JOIN Results re ON re.runner\_id = r.runner\_id INNER JOIN  Training_Plans tp ON tp.runner\_id = re.runner\_id INNER JOIN Training_Plans_Descriptions tpd ON tpd.plan\_id  = tp.plan\_id 
INNER JOIN Events e ON e.event_id = re.event_id WHERE 
event = 'London Marathon' AND event\_date = '2016-04-24'
ORDER BY re.finish_time ASC LIMIT 10;}


WHERE tp.event\_id IN (SELECT e.event\_id FROM Events e WHERE event = 'London Marathon' AND event\_date = '2016-04-24') ORDER BY re.finish_time ASC LIMIT 10;}
...
Three INNER JOINs are done to join Runners to Results, Results to Training_Plans, Training_Plans to Training_Plans_Descriptions. 
